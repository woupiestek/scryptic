...
export class Compiler {
  ...
  #boolean(
    ...
    switch (this.#tokenType(node)) {
      ...
      case TokenType.OR:
        this.#booleanBinary(expression, expression.pop() ?? -1, onFalse);
        return;
  ...
  #binary(lhs: number[], rest: number): Register {
    switch (this.#tokenType(rest)) {
      case TokenType.BE:
        // return this.#assignment(lhs, rest);
      case TokenType.AND:
      case TokenType.IS_NOT:
      case TokenType.IS:
      case TokenType.LESS:
  ...
  #expression(
    ...
    switch (this.#tokenType(node)) {
      ...
      case TokenType.PAREN_LEFT:
        ...
        if(nodes.length===0) return this.#expression(this.#children(node).toArray());
  ...
  *#children(node: number) {
  ...
  #class(declaration: number) {
    const klaz = this.classes[this.#lexeme(declaration)] ||= new Class();
    for (const methodDeclaration of this.#children(declaration)) {
      this.#method(
        methodDeclaration,
  ...
  #block(block: number) {
    ...
    for (const statement of this.#children(block + 1)) {
  ...
  #statement(statement: number) {
    switch (this.#tokenType(statement)) {
      case TokenType.BRACE_LEFT:
        ...
        return;
      case TokenType.IF: {
        const [condition, onTrue, onFalse] = this.#children(statement);
        const continuation = new Label(this.#next());
      ...
      case TokenType.WHILE: {
        const [condition, onTrue] = this.#children(statement);
        const continuation = new Label(this.#next());
      ...
      default:
        this.#repay(this.#expression(this.#children(statement + 1).toArray()));
        return;
    }
  }
